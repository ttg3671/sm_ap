...
import { Outer, Navbar, Card, TagList } from "../../components";
    ...
    const handleDelete = async (id, type) => {
        ...
        try {
            ...
            if (response.data?.isSuccess) {
                setSliderList((prev) =>
                    prev.filter((item) => item.id !== id && Number(item.category_id) !== Number(type))
                );
                setIsLoading(false);
            }
        } 
    ...
    useEffect(() => {
        ...
        const getTag = async () => {
            try {
                ...
                if (isMounted && response.data?.isSuccess) {
                    setSliderList(response.data?.data?.result || []);
                ...
                <div className="col-span-full flex flex-col items-center justify-start pt-24 px-4 w-full">
                    ...
                    {isLoading ? (
                        <div className="relative z-10 flex justify-center items-center h-50 pt-10">
                            ...
                            <>
                                <TagList tags={tagList} handleClick={handleClick} />
